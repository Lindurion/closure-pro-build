 ...
 *     alwaysLoadedAfterModules: !Array.<string>} Dependency-ordered list of
...
ModuleManager.prototype.initModules = function(projectOptions) {
  ...
  this.maybeAddVirtualBaseModule(projectOptions);
};
 ...
 * @param {!Object} projectOptions
 */
ModuleManager.prototype.maybeAddVirtualBaseModule = function(projectOptions) {
  ...
  // to all root modules.
  projectOptions.jsModules[VIRTUAL_BASE_MODULE] = {
...
ModuleManager.prototype.calcInputs = function() {
  ...
  for (var moduleId in this.modules) {
    inputsByModule[moduleId] = {
      ...
      compiledInputFiles: [],
      dontCompileInputFiles: [],
      alwaysLoadedAfterModules: this.getDepModuleNames(moduleId)
    };
  }
 ...
 * @return {!Array.<string>} List of module names the given module transitively
 *     depends on (but this time, not including self).
 */
ModuleManager.prototype.getDepModuleNames = function(moduleId) {
  var depModuleNames = [];
  for (var depModuleId in this.modules[moduleId].transitiveModuleDeps) {
    if (depModuleId != moduleId) {
      depModuleNames.push(this.modules[depModuleId].name);
    }
  }
